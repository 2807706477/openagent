...
class UserService:
    ...
    def create_user(self, user_data: UserCreate) -> User:
        ...
        try:
             # --- 🔍 新增调试代码开始 ---
            logger.error(f"🚨 DEBUG: 接收到的 user_data 类型: {type(user_data)}")
            logger.error(f"🚨 DEBUG: user_data 原始内容: {user_data}")
            
            # 关键：打印 password 字段的真实值和类型
            pwd_val = user_data.password
            logger.error(f"🚨 DEBUG: 准备哈希的 password 值: [{pwd_val}]")
            logger.error(f"🚨 DEBUG: password 类型: {type(pwd_val)}")
            logger.error(f"🚨 DEBUG: password 长度: {len(str(pwd_val)) if pwd_val else 0}")
            # --- 🔍 新增调试代码结束 ---
            # Validate input
